...
async def account_login(bot: Client, m: Message):
    ...
    if user is not None and user not in sudo_users:
        ...
        editable = await m.reply_text(
            "Hello Bro **I am Text Downloader Bot and made by 🅼🆁 🅸🅽🅳🅸🅐**. I can download videos from **text** file one by one.**\n\nDeveloper** : 🅼🆁 🅸🅽🅳🅸🅐**\nLanguage** : Python**\nFramework** : Pyrogram\n\nSend **TXT** File FORMAT {FileName : FileLink}")
...
async def account_login(bot: Client, m: Message):
    ...
    if user is not None and user not in sudo_users:
        ...
        editable = await m.reply_text(
            "Hello Bro **I am top Downloader Bot and made by 🅼🆁 🅸🅽🅳🅸🅐**. I can download videos from **text** file one by one.**\n\nLanguage** : Python**\nFramework** : Pyrogram\n\nSend **TXT** File {Name : Link}"
       ,reply_markup=keyboard)
    input: Message = await bot.listen(editable.chat.id)
    x = await input.download()
    await input.delete(True)
